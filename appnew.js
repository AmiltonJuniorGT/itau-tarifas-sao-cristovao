...
function bindImport(){
  ...
  if(fileInput){
    fileInput.addEventListener("change", () => {
      ...
      if(SELECTED_FILE){
        ...
        setStatus(`Arquivo selecionado. Unidade identificada: ${CURRENT_UNIT}. Clique em Processar e padronizar.`);
      }
    });
  }
  ...
  if(dropzone && fileInput){
    ...
    dropzone.addEventListener("drop", e => {
      ...
      setStatus(`Arquivo arrastado. Unidade identificada: ${CURRENT_UNIT}. Clique em Processar e padronizar.`);
    });
  }
}
...
/* =========================================================
   DETECÇÃO AUTOMÁTICA DA UNIDADE PELO NOME DO ARQUIVO
========================================================= */
function detectUnitFromFilename(filename){
  ...
  if(name.includes("sao cristovao") || name.includes("gtsc")) return "São Cristóvão BA";
  if(name.includes("cajazeiras") || name.includes("gtcaja")) return "Cajazeiras BA";
  if(name.includes("camacari") || name.includes("gtcam")) return "Camaçari BA";
  if(name.includes("gama") || name.includes("gtgama")) return "Gama - DF";
...
async function processFile(){
  try{
    ...
    setStatus(`Base padronizada para ${CURRENT_UNIT}: ${BASE_RAW.length.toLocaleString("pt-BR")} lançamentos e ${DENOM_SUMMARY.length.toLocaleString("pt-BR")} denominações.`);
...
function parseItauRows(rows){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    raw.push({
      Data:formatDateBR(data), DataISO:toISODate(data), Unidade:CURRENT_UNIT, Banco:"Itaú", Conta:"",
...
function resetAll(){
  BASE_RAW=[]; BASE_SELECTED=[]; DENOM_SUMMARY=[]; SELECTED_DENOMS=new Set(); SELECTED_FILE=null; CURRENT_UNIT="Unidade não identificada";
