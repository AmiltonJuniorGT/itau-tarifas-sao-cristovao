/* =========================================================
   GT BANK ANALYTICS — app.js v3.3 FINAL
   Correção principal:
...
const APP_VERSION = "3.3-unidade-final-2026-07-13";
...
function initApp(){
  try{
    ...
    setStatus(`Pronto (${APP_VERSION}). Selecione um arquivo e clique em Processar e padronizar.`);
...
async function processFile(){
  try{
    ...
    // PRIORIDADE ABSOLUTA: nome do arquivo.
    // O conteúdo interno só é usado quando o nome não identifica a unidade.
    const unitFromFilename = detectUnitFromFilename(file.name);
    const unitFromRows = detectUnitFromRows(rows);

    CURRENT_UNIT = unitFromFilename !== "Unidade não identificada"
      ? unitFromFilename
      : unitFromRows;
    ...
    setStatus(
      ...
      `Unidade confirmada: ${CURRENT_UNIT}. Versão: ${APP_VERSION}. Padronizando...`
    );

    BASE_RAW = parseItauRows(rows, CURRENT_UNIT)
      .map(item => ({ ...item, Unidade: CURRENT_UNIT }));

    // Proteção final: nenhuma linha pode manter unidade antiga ou fixa.
    const wrongUnit = BASE_RAW.find(item => item.Unidade !== CURRENT_UNIT);
    if(wrongUnit){
      throw new Error("Falha interna ao aplicar a unidade identificada.");
    }
...
function resetAll(){
  ...
  setStatus(`Aguardando arquivo... (${APP_VERSION})`);
